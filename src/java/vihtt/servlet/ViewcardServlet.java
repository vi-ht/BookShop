...
final String VIEW_PAGE="viewcard";
         /**
        ...
        try {
            ...
            List<ItemDTO> listBookInCart = new ArrayList<>();
                 for (String itemID : item.keySet()) {
                          ItemDTO dto = dao.getBookByID(itemID,item.get(itemID));
                          if(dto!=null){
                                   listBookInCart.add(dto);
                          }
                 }
                 request.setAttribute("LISTBOOKINCART", listBookInCart);
        } catch (NumberFormatException e){
                 log("ViewcardServlet_NumberFormat"+e.getMessage());
        } catch (SQLException e){
                log("ViewcardServlet_SQL"+e.getMessage());
        } catch (NamingException e){
                log("ViewcardServlet_Naming"+e.getMessage());
        }finally{
